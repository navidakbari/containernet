    ...
    def __init__(self, name, dimage=None, dcmd=None, build_params={},
        ...
        * updateMemoryLimits(...)
        ...
        defaults = {'cpu_quota': -1,
                    ...
                    'ipc_mode': None,
                    'devices': [],
                    # we need this to allow mininet network setup
                    'cap_add': ['net_admin'],
                    'storage_opt': None,
                    'sysctls': {}
                    }
        ...
        if 'net_admin' not in defaults['cap_add']:
            # adding net_admin if it's cleared out to allow mininet network setup
    ...
    def _image_exists(self, repo, tag, _id=None):
        ...
        for image in images:
            ...
            if image.get("Id", None):
                debug("; ".join([str(repo), str(tag),
                      str(_id), str(image.get("Id"))]))
                if image.get("Id") == _id:
                    return True
        return False
...
class RunningDocker (Host):
    def __init__(self, containerName, dimage=None, dcmd=None, build_params={},
                 **kwargs):
        """
        Creates a Docker container as Mininet host.
        ...
        * updateMemoryLimits(...)
        ...
        # fetch information about new container
        self.dcinfo = self.dcli.inspect_container(self.containerName)
        self.did = self.dcinfo.get("Id")

        # call original Node.__init__
        Host.__init__(self, containerName, **kwargs)
...
class OVSSwitch(Switch):
    ...
    def attachInternalIntf(self, intf_name, net):
        ...
        self.vsctl('add-port', self.deployed_name, intf_name, '--',
                   'set', ' interface', intf_name, 'type=internal')
